...
router.get('/', (req, res) => {
    ...
    const songs = query(db, 'SELECT * FROM songs ORDER BY sort_order ASC, id ASC');
    res.json(songs);
});

router.get('/:id', (req, res) => {
    const db = req.app.locals.db;
    const songs = query(db, 'SELECT * FROM songs WHERE id = ?', [Number(req.params.id)]);
...
router.post('/', authMiddleware, async (req, res) => {
    ...
    const maxOrder = query(db, 'SELECT MAX(sort_order) as m FROM songs');
    const nextOrder = (maxOrder[0]?.m || 0) + 1;

    run(db, 'INSERT INTO songs (title, artist, folder, filename, lrc_filename, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
...
router.put('/:id', authMiddleware, (req, res) => {
    ...
    run(db, 'UPDATE songs SET title = ?, artist = ?, sort_order = ? WHERE id = ?',
...
router.delete('/:id', authMiddleware, (req, res) => {
    ...
    run(db, 'DELETE FROM songs WHERE id = ?', [Number(req.params.id)]);
    saveDB(db);
    res.json({ success: true });
});
...
router.post('/reorder', authMiddleware, (req, res) => {
    ...
    order.forEach((id, i) => {
        run(db, 'UPDATE songs SET sort_order = ? WHERE id = ?', [i, id]);
    });
    saveDB(db);
    res.json({ success: true });
});
