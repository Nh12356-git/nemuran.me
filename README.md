...
| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | /music | 上传音乐（多文件，仅音频） | ✓ |
| POST | /picture | 上传图片（仅图片） | ✓ |
...
- 启动时自动扫描 `file/music/` 和 `file/picture/` 目录，将已有文件导入数据库
- 底栏工具图标自动从目标网站获取 favicon
- 图片预览支持滚轮缩放（0.2x~5x）、拖拽移动、ESC/点击外部关闭
- 文件上传自动校验类型（仅允许图片/音频）
